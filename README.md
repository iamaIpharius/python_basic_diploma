...
##### **Модуль Commands**
---
**api_key** - Ключ HotelsAPI 

* **get_destination_id** - Получаем ID отеля
* **hotels_list_by** - Список отелей, отсортированный по самой низкой цене либо по самой высокой цене
* **hotels_list_bestdeal** - Список отелей по команде bestdeal
* **get_clean_list** - Получить отсортированный список для команды bestdeal, используется только внутри функции **hotels_list_bestdeal**
* **get_photos** - Получить фотографии отеля по ID
* **form_result_string** - Сформировать итоговое сообщение из мнформации об отеле
* **form_history** - Сформирвать сообщение с историей

## Использование бота в Телеграмм
---
### Команды
1. **start** - запуск бота, приветствие, возвращает список команд для использования бота
2. **help** - команада возвращает список команд для использования бота
3. **lowprice** - поиск отелей по самой низкой цене
4. **highprice** - описк отелей по самой высокой цене
5. **bestdeal** - поиск отелей по критериям цены и расстояния до центра города
6. **history** - возвращает историю запросов и результатов, полученных пользователем
